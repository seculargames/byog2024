<script lang="ts">
    ...
    function initializeGameCanvas(canvas) {
        const style = canvas.style('.mycolor', { color: 'pink' });
        canvas.add(style);
        let randMap = engine.gm();
        for (let i=0; i++; i < randMap.city.length){
    ...
    gameState.subscribe((value) => {
        ...
        //\let tot_energy = mean(value.user.energy);
        //console.log(tot_energy);
        //if (value.user.health <= 0 || tot_energy <= 0) {
        //  goto('/deadpage');
        //}
    });
